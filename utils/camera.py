...
def to_hom(X):
    ...
    return X_hom
def translation_dist(t1,t2):
    error=torch.acos(torch.sum(t1*t2)/t1.norm()/t2.norm())/torch.pi*180
    return error
...
def get_center_and_ray(opt,pose,intr=None,rays_idx=None,xy_grid=None): # [HW,2]
...
def get_novel_view_poses(opt,pose_anchor,N=60,scale=1):
    ...
    R_x = angle_to_rotation_matrix((theta.sin()*0.008).asin(),"X")
    R_y = angle_to_rotation_matrix((theta.cos()*0.008).asin(),"Y")
    pose_rot = pose(R=R_y@R_x)
    pose_shift = pose(t=[0,0,-0.5*scale])
    pose_shift2 = pose(t=[0,0,0.2*scale])
