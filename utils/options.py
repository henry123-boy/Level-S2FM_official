...
def parse_arguments(args):
    ...
    for arg in args:
        ...
        if "=" not in arg[2:]:
            key_str,value = (arg[2:-1],"false") if arg[-1]=="!" else (arg[2:],"true")       # for example: --barf --barf!, the former means that barf==True
        else:
            key_str,value = arg[2:].split("=")
        keys_sub = key_str.split(".")
        opt_sub = opt_cmd
        for k in keys_sub[:-1]:
            if k not in opt_sub: opt_sub[k] = {}      # here, actually, the value of opt_cmd have been changed, meanwhile opt_sub is only like a temporary point
            opt_sub = opt_sub[k]
